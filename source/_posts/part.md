...
- 组件是按照一些小功能的通用性和可复用性抽象出来的，可以跨项目
...
## 容器型组件
可以理解为当前功能模块最外层的父组件即某块儿功能入口，集中/统一的状态管理，向其他展示型/容器型组件提供数据（充当数据源）和行为逻辑处理（接收回调），还可以包括处理一些该页面中不属于任何一个展示组件的方法
