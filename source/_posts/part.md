...
<font color=red>此时，已经带有一点组件化的味道</font>
...
是一个比较完整的组件化方案了，但却没能够进入标准，默默地消失了，今天的角度来看，它可以说是生不逢时
...
分子是由原子组成的，分子分成原子，原子也可以重新组合成新的分子”一个界面是由独立的分子组件搭建而成，分子组件由原子元件构成，这些原子可通过不同的组合方式，组成新分子组件，继而重组构成新的界面
...
* 现代组件化方案虽然稍显不同，但都尽量保留了原有的标记语言部分与样式表部分
* 组件化是对实现的分层，是更有效地代码组合方式
* 组件化是对资源的重组和优化，从而使项目资源管理更合理
* 组件化有利于单元测试
* 组件化对重构较友好
* 技术一直在变迁，但组件化的核心并没有改变，目标仍然是在API设计尽可能接近原生的情况下完成复用、解耦、封装、抽象的目标，最终服务于开发，提高效率，降低错误率
* 组件化就是基于可重用的目的 将一个大的软件系统按照关注点的形式 拆分为多个独立的组件 主要的目的就是减少耦合 
* 这种化繁为简的思想在后端开发中的体现是微服务，而在前端开发中的体现就是组件化
* 通常来讲，组件是由一系列模块组成的带有生命周期的有机整体。反过来，一个大的模块，也是可以包含一系列的组件
...
随着项目规模的扩大，组件之间的依赖关系会发生改变，我们需要对组件中的环形结构进行监控，消除依赖环
...
## 容器型组件
一个容器性质的组件，可以理解为最外层的父组件即某块儿功能入口，集中/统一的状态管理，向其他展示型/容器型组件提供数据（充当数据源）和行为逻辑处理（接收回调），还可以包括处理一些该页面中不属于任何一个展示组件的方法

它有以下这些特点：

- 只负责业务逻辑和数据的处理
- 基本上涵盖了本模块的业务逻辑，主要表现为组件怎么工作的 数据怎么更新的不包含任何`Virtual Dom`的修改或组合，也不会包含组件的样式
- 如果使用了状态管理工具（redux/vuex），那它就是与`store`通信的唯一入口
- 充当子级组件通信的状态中转站
...
```
## 业务组件
话不多说，包含你具体业务逻辑的组件，先有业务组件，再谈通用组件
## 通用组件
可以在一个或多个APP内通用的组件
### UI组件
...
React中高阶组件是比较常用的组件封装形式，Vue官方内置了一个高阶组件[keep-alive](https://github.com/vuejs/vue/blob/dev/src/core/components/keep-alive.js)通过维护一个cache实现数据持久化，但并未推荐使用HOC :(

```
* 在 React 中写组件就是在写函数，函数拥有的功能组件都有

* Vue 更像是高度封装的函数，能够让你轻松的完成一些事情，但与高度的封装相对的就是损失一定的灵活，你需要按照一定规则才能使系统更好的运行
```
...
理想情况下，组件的引用层级只有两级，业务组件引用通用组件，我们在容器组件里去自由组合这些组件


![image](http://pw1zvypwt.bkt.clouddn.com/zujainxietong.png)
...
物极必反，在思考一个组件时，思考以下几个问题，引导完善组件的设计
...
通⽤组件一定是与业务解耦又要服务于业务开发的,那么问题来了,如何保证组件的通⽤性,通⽤性⾼⼀定是好事吗?
